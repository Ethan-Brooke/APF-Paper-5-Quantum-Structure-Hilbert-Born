...
**You are reading the companion to Paper 5** (this repo: https://github.com/Ethan-Brooke/APF-Paper-5-Quantum-Structure-Hilbert-Born).
...
When a user's question is about a result outside Paper 5, point them to the relevant row above. Do not attempt to re-derive cross-paper results from the bundled subset — the canonical state is the codebase ([Zenodo concept DOI](https://doi.org/10.5281/zenodo.18529115)), not this slice.

---

## 1. What this repository is

This is the **paper-companion release repo for Paper 5 of the Admissibility Physics Framework (APF) series**. Specifically: Quantum Structure from Finite Enforceability.

The repo bundles:

- The manuscript and Technical Supplement (`.tex` + `.pdf`)
- A vendored subset of the APF codebase containing only the `check_*` functions that Paper 5 directly references (`apf/core.py`, `apf/apf_utils.py`, `apf/bank.py`)
...
The framework-wide canonical codebase (342 bank-registered theorems, 19 modules, 355 `verify_all` checks) is **not** bundled here. Only the Paper 5 subset is runnable locally. See `ai_context/THEOREMS.md` for the full bank listing with bundled-in-this-repo flags.
...
If you're loading this repo cold and want to understand Paper 5 well enough to reason about it:
...
7. **`Paper_5_Quantum_Structure_Hilbert_Born_v2.0-PLEC.tex`** or `.pdf` — the manuscript. Read §1 (intro) and the section you care about. Don't try to read the whole paper first.
