# Reviewers' Guide — APF Paper 5: Quantum Structure from Finite Enforceability

This guide is the physics-first walkthrough for peer reviewers of Quantum Structure from Finite Enforceability. It addresses the structural assumptions, anticipated objections, and falsification surfaces of the paper's argument. The intended audience is a peer reviewer with a background in physics or mathematics.

The guide complements three other verification routes:
- The [executable codebase](apf/) (run `python run_checks.py` for the full 8-theorem verification);
- The [Colab notebook](APF_Reviewer_Walkthrough.ipynb) (zero-install, theorem-by-theorem cells with prose + code);
- The [interactive derivation DAG](https://ethan-brooke.github.io/Quantum-Structure-Hilbert-Born/) (hover-for-details, click-for-dependencies, animated verification).

---

## The derivation in 8 steps

The full chain from the foundational commitments to this paper's results:

**Step 1 — L_ST_algebra** (`check_L_ST_algebra`)

  L_ST_algebra: Connes Finite Algebra A_F = C ⊕ M_2(C) ⊕ M_3(C) from T_gauge [P].


**Step 2 — L_Gleason_finite** (`check_L_Gleason_finite`)
...
To prevent scope inflation, the following are flagged explicitly as *outside* the scope of Quantum Structure Hilbert Born:
...
- **`apf/core.py`** — the 8 theorem check functions for this paper. Each function constructs a mathematical witness, verifies the theorem's claim, and returns a structured result with name, dependencies, status, and key result.
...
This repository is the executable mathematical appendix to APF Paper 5. The canonical archival deposit is at [https://doi.org/10.5281/zenodo.18604861](https://doi.org/10.5281/zenodo.18604861) (DOI: 10.5281/zenodo.18604861).
